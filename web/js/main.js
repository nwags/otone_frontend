...
function calibrate (axis, property, current) {
  ...
  if(robotState.pipettes[axis] && !isNaN(robotState.pipettes[axis][property])) {
    ...
    if(property==='bottom') {
      var blowout_offset = 2;
      if (axis == 'a') {
        blowout_offset = document.getElementById('blowout_offset_a').value;
      }
      if (axis == 'b') {
        blowout_offset = document.getElementById('blowout_offset_b').value;
      }
      robotState.pipettes[axis]['blowout'] = robotState.pipettes[axis]['bottom'] + blowout_offset;
      var msg_blowout = {
        'type' : 'calibratePipette',
        'data' : {
          'axis' : axis,
          'property' : 'blowout_offset',
          'value' : blowout_offset
        }
      };
      sendMessage(msg_blowout);
    }
  }
}
