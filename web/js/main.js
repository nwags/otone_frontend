...
window.addEventListener ('load', function () {
  ...
  connection.onopen = function(session) {
    ...
    connection.session.subscribe('com.opentrons.robot_ready', function(data){
      //status
      console.log('data: ',data);
      var status = false;
      if (data.hasOwnProperty("status")){
        console.log('data has status');
        status = data.status;
      }
      console.log('data does not have status');

      if (data.hasOwnProperty("uuid")){
        uuid_cookie = data.uuid;
      }
    ...
    uuid_cookie = getCookie('uuid');
    console.log('uuid_cookie: ',uuid_cookie);
    if (uuid_cookie) {
      console.log('yes cookie');
      url_topic += uuid_cookie;
      connection.session.publish(url_topic,[true]);
    } else {
      console.log('no cookie');
      if (uuid_cookie) {
        setCookie(uuid_cookie);
      }
...
function selectContainer(currentDiv) {
  ...
  if(currentDiv) {
    ...
    console.log("TIPRACKS['a'] - 1: ",TIPRACKS['a']);
    console.log("TIPRACKS['b'] - 1: ",TIPRACKS['b']);
...
function saveContainer (axis) {
  ...
  if(axis == 'a'){
    ...
    console.log('TIPRACKS[a][0] - 2: '+TIPRACKS['a'][0]);
    ...
    moveBtn.style.display = 'inline-block';
    console.log('TIPRACKS[b][0] - 2: '+TIPRACKS['b'][0]);
...
var socketHandler = {
  ...
  'current_protocol' : function(data) {
    if (data.file_name && data.current_protocol){
      var file = new File([JSON.stringify(data.current_protocol)],data.file_name)
      loadFile(file);
    }
  }
};
